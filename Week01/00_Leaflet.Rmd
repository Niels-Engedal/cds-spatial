...
leaflet() %>% 
  ...
  addProviderTiles("Esri.WorldPhysical", group = "Physical", options = providerTileOptions(opacity=0.25)) %>% 
...
leaflet() %>%
  ...
  addProviderTiles("Esri.WorldImagery", 
                   options = providerTileOptions(opacity=0.5)) %>% 
  setView(lng = 9, lat = 56, zoom = 6)

l_dk <- leaflet() %>% setView(lng = 9, lat = 56, zoom = 6)


# Let's get all the maps provider layers from Esri:
esri <- grep("^Esri", providers, value = TRUE)
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit?gid=992055308#gid=992055308",
                     range = "SA2026",
                     col_types = "cccnncnc")
glimpse(places)
```
...
# YOUR CODE
places <- places %>%
  mutate(PopupLabel = paste0("<b>", Placename, "</b><br>", Description))

DKmap <- DKmap %>% addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = places$PopupLabel,
...
```{r - save-DKmap-with-labels}
library(htmlwidgets)
saveWidget(DKmap, "DKmap_with_places.html", selfcontained = TRUE)
```
