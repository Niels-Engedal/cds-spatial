...
wcdata <- wc %>% 
  ...
  summarize(flatSum = sum(flats), #total number of flats by suburb
          ...
          #percentage of flats with baths in a suburb
            wcSum= sum(wc_access)) %>%   #total number of toilets by suburb
  mutate(pct_bath = bathSum / flatSum * 100,
          pct_wc = wcSum/ flatSum *100) #percentage of flats with toilets in a suburb
...
## YOUR CODE
library(sfarrow)
servants <- st_read_parquet("../data/Servants.parquet")
unique(servants$quarter)

library(mapview)
servants %>% 
  filter(quarter == "Kobenhavn O") %>% 
  mapview()
...
mapview(baths_cph[,-3], 
        ...
        cex="radius", legend=FALSE, col.regions="#217844", lwd=0, alpha=0.4)
